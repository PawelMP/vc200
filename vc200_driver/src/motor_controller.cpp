...
MotorController::MotorController(std::shared_ptr<STInterface::STInterfaceClientUDP> st_if, ros::NodeHandle &nh)
  ...
  //max_command = 100.0; ??? Changed to 1000.0
  max_command = 1000.0;
...
void MotorController::readData() {
  ...
  leftJointState.position += -encData.leftSideDistance.value;//(encData.leftSideDistance.int_value / (float)encoder_resolution) * 2.0 * M_PI;

  rightJointState.effort = motorData.rightWheelPwm;
  rightJointState.velocity = encData.rightSideVelocity.value;
  rightJointState.position += encData.rightSideDistance.value;//(encData.rightSideDistance.int_value / (float)encoder_resolution) * 2.0 * M_PI;
}
