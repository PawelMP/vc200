...
bool VC200Driver::init(ros::NodeHandle &nh, ros::NodeHandle &priv_nh) {
  ...
  try {
    ...
    //Ultrasound
    ultrasoundPtr_.reset(new Ultrasound(stClientPtr_, priv_nh)); 
    //Minilidar
    minilidarPtr_.reset(new Minilidar(stClientPtr_, priv_nh)); 
